...
small_screen_plot <- ggraph(graph, layout = layout) +
  geom_edge_link(aes(color = "darkorange"), 
                 show.legend = FALSE,
                 start_cap = circle(2, "mm"),  # Reduce the size of start_cap
                 end_cap = circle(2, "mm")) +  # Remove arrow from edges
  geom_node_point(aes(color = as.factor(id)), size = 5) +  # Reduce the size of node points
  geom_node_text(aes(label = str_wrap(label, width = 8)), vjust = -0.5, hjust = 0.5, size = 2) +  # Adjust text wrapping
  geom_node_text(aes(label = prices), vjust = -1, hjust = 0.5, size = 2, color = "black") +  # Adjust the position and size of price labels
  geom_edge_label(aes(label = desired_ratings, filter = edge_size > 6), vjust = 0, hjust = 0, size = 3, color = "black") +  # Add edge labels with a filter condition
  theme_void() +
  theme(

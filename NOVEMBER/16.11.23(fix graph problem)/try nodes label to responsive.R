...
library(ggplot2)
library(shinyjs)
library(jsonlite)  # Make sure to load the jsonlite library

ui <- fluidPage(
  useShinyjs(),
  fluidRow(
    column(width = 12,
           plotOutput("graphPlot", height = "700px", width = "100%")
    )
  )
)
...
server <- function(input, output) {
  output$graphPlot <- renderPlot({
    ...
    ggraph(graph, layout = layout) +
      ...
      geom_node_text(aes(label = str_wrap(label, width = input$label_width)), vjust = 0.2, hjust = 0.5, size = 2.5) +
  ...
  observe({
    shinyjs::runjs("
      var labelWidth = Math.min(12, Math.floor($(window).width() / 20));
      Shiny.setInputValue('label_width', labelWidth, {priority: 'event'});
    ")
  })
}

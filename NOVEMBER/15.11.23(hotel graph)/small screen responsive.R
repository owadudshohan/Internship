...
small_screen_plot <- ggraph(graph, layout = layout) +
  geom_edge_link(aes(label = desired_ratings, vjust = 0.6, hjust = -0.7, color = "darkorange"), 
                 show.legend = FALSE,
                 start_cap = circle(3, "mm"),  # Reduce the size of start_cap
                 end_cap = circle(3, "mm")) +  # Reduce the size of end_cap
  geom_node_point(aes(color = as.factor(id)), size = 10) +  # Reduce the size of node points
  geom_node_text(aes(label = str_wrap(label, width = 8)), vjust = 0.2, hjust = 0.5, size = 2) +  # Reduce the font size and adjust text wrapping
  geom_node_text(aes(label = prices), vjust = 1.5, hjust = 0.5, size = 2, color = "black") +  # Adjust the position and size of price labels
  theme_void() +
  theme(

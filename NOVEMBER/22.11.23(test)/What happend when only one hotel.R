...
server <- function(input, output) {
  output$graphPlot <- renderPlot({
    # Define the new data
    hotelName <- c("Hotel A bla  bla bla")
    ...
    additional_text <- ifelse(nodes$label == targetHotel, "r-2.5", "")
    ...
    ggraph(graph, layout = layout) +
      geom_edge_link(aes(label = desired_ratings, vjust = 1, hjust = -0.3, color = "darkorange"), 
                     ...
                     end_cap = circle(5, "mm")) +
      geom_node_point(aes(color = as.factor(id)), size = ifelse(nodes$id == rowIndexOfTargetHotel, 50, 20)) +
      geom_node_text(aes(label = str_wrap(nodes$label, width = 12)), vjust = 0.2, hjust = 0.5, size = ifelse(nodes$id == rowIndexOfTargetHotel, 5.2, 2.5)) +
      geom_node_text(aes(label = prices), vjust = 2, hjust = 0.5, color = "black", size = ifelse(nodes$id == rowIndexOfTargetHotel, 5.2, 2.5)) +
      geom_node_text(aes(label = additional_text), vjust = 3.1, hjust =0.5, size = 5.5, color = "red") +
      theme_void() +
      theme(

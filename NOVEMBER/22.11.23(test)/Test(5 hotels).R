...
server <- function(input, output) {
  output$graphPlot <- renderPlot({
    ...
    desired_ratings <- c("r-2.5", "r-4", "r-6", "r-3")
    
    # Additional text for "7.5"
    additional_text <- ifelse(nodes$label == targetHotel, "r-4.5", "")
    
    # Plotting using ggplot2
    ggraph(graph, layout = layout) +
      geom_edge_link(aes(label = desired_ratings, vjust = 1, hjust = -0.7, color = "darkorange"), 
                     show.legend = FALSE,

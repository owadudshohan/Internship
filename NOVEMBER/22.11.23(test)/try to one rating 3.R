  ...
  output$graphPlot <- renderPlot({
    ...
    ggraph(graph, layout = layout) +
      geom_edge_link(aes(label = rating, vjust = -0.4, hjust = -0.7, color = "darkorange"), 
                     ...
                     end_cap = circle(5, "mm")) +
      geom_node_point(aes(color = as.factor(id)), size = ifelse(nodes$id == rowIndexOfTargetHotel, 31, 25)) +
      geom_node_text(aes(label = str_wrap(nodes$label, width = 14)), vjust = 0.2, hjust = 0.5, size = ifelse(nodes$id == rowIndexOfTargetHotel, 3.6, 3.2)) +

library(shiny)
library(tidygraph)
#It allows for a tidy data approach to graph manipulation, analysis, and visualization.
library(igraph)
#used for more advanced graph operations and calculations.
library(ggraph)
# works with tidygraph and provides a grammar of graphics approach to graph visualization.
library(tibble)
#It's used to create and manipulate tabular data.
library(stringr)
# It's likely used for text processing, such as wrapping text in labels or extracting substrings.
library(ggplot2)
...
ui <- fluidPage(
  fluidRow(
    column(width = 12,
           ...
           #The width = 12 indicates that the entire width of the row is allocated to this column.
    )
  )
)

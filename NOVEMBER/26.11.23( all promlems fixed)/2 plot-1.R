...
hotelInfo <- tibble(
  ...
  existingRating = c("2.5", "4", "6", "7.5", "2", "8", "7"),
...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    column(
      width = 12,
      plotOutput("singleRoomPlot", height = "600px", width = "100%")
    ),
    column(
      width = 12,
      plotOutput("doubleRoomPlot", height = "600px", width = "100%")
    )
  )
)
...
server <- function(input, output) {
  generatePlot <- function(targetHotel, priceRoom) {
    ...
    edgeRatings <- hotelInfo$existingRating[-rowIndexOfTargetHotel]
    ...
    gg <- ggraph(graph, layout = layout) +
      ...
      annotate("text", x = layout[rowIndexOfTargetHotel, 1], y = layout[rowIndexOfTargetHotel, 2],
               label = hotelInfo$existingRating[rowIndexOfTargetHotel],vjust = 4.4, hjust = 0.5, size = ifelse(length(hotelInfo$hotelName) == 2, 3, 3.6), color = "red") +
      theme_void() +
      theme(

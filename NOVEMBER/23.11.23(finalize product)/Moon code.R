  ...
  output$graphPlot <- renderPlot({
    ...
    ggraph(graph, layout = layout) +
      ...
      annotate("text", x = layout[rowIndexOfTargetHotel, 1], y = layout[rowIndexOfTargetHotel, 2],
               # label = existingRating[rowIndexOfTargetHotel], vjust = 4.4, hjust = 0.5, size = 3, color = "red") + # for2 hotels
               label = existingRating[rowIndexOfTargetHotel],vjust = 4.4, hjust = 0.5, size = 3.6, color = "red") +  # Use annotate to add text label for central node's rating
      theme_void() +
      theme(
